...
<p>
<button type="button" onclick="window.location='<?php echo $admin_path; ?>';" class="<?php echo tcp_get_buy_button_color(); ?> tcp-btn-sm"><?php _e( 'Create new address', 'tcp' ); ?></button>
</p>
	...
	<tr>
		...
		<td style="width: 20%;">
			<div class="btn-group btn-group-xs">
				<button type="button" onclick="window.location='<?php echo add_query_arg( 'address_id', $address->address_id, $admin_path ); ?>'" class="btn btn-default" title="<?php _e( 'edit', 'tcp' ); ?>"><span class="glyphicon glyphicon-pencil"></span> <span class="sr-only"><?php _e( 'edit', 'tcp' ); ?></span></button>
				<button type="button" onclick="jQuery('.delete_address').hide();jQuery('#delete_<?php echo $address->address_id; ?>').show();return false;" class="btn btn-default" title="<?php _e( 'delete', 'tcp' ); ?>"><span class="glyphicon glyphicon-trash"></span> <span class="sr-only"><?php _e( 'delete', 'tcp' ); ?></span></button>
			</div>
			<div id="delete_<?php echo $address->address_id; ?>" class="delete_address" style="display:none; border: 1px dotted orange; padding: 2px">
				<form method="post" name="frm_delete_<?php echo $address->address_id; ?>">
					...
					<div class="btn-group btn-group-xs">
						<button type="button" onclick="document.frm_delete_<?php echo $address->address_id; ?>.submit();" class="btn btn-default" title="<?php _e( 'Yes' , 'tcp' ); ?>"><span class="glyphicon glyphicon-ok"></span></button>
						<button type="button" onclick="jQuery('#delete_<?php echo $address->address_id; ?>').hide();return false;" class="btn btn-default" title="<?php _e( 'No, I don\'t' , 'tcp' ); ?>"><span class="glyphicon glyphicon-remove"></span></button>
					</div>
				</form>
			</div>
		</td>
